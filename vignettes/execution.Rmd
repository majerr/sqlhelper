...
## read_example <- read_sql("examples/read_example.sql")

```

## Executing SQL strings

One of the main objectives of SQL helper is to reduce the incidence of SQL
...
It may not always be possible or desire-able to have `sqlhelper` manage your
database connections. For example, the use of secrets is not yet supported in
`sqlhelper` connections. In these cases, you may pass your own connections to
`run_files()` or `run_queries()`.
